...
#import "TeacherPersonVC.h"
#import "LoginVC.h"
@interface TeacherPersonVC ()<UIGestureRecognizerDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
@implementation TeacherPersonVC
...
- (void)viewDidLoad {
    ...
    NSString *headImagePath = [[NSUserDefaults standardUserDefaults]objectForKey:@"headPhoto_tea"];
    [_headImageView setImage:[UIImage imageNamed:@"testIcon"]];
    if (headImagePath&&headImagePath.length>0) {
        [_headImageView setImage:[self takepicture:[NSString stringWithFormat:@"%ld",userid+100]]];
    }
    
    
    
    [self.nameL setText:model.teacher_name];
    [self.yuanxiL setText:model.teacher_department];
    [self.positionL setText:model.teacher_title];
    [self.phoneL setText:model.teacher_phone];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [self saveImage:tempImage name:[NSString stringWithFormat:@"%ld",userid+100]];
}
//修改图像的尺寸
...
- (void)saveImage:(UIImage *)image name:(NSString *)iconName
{
    ...
    [userDe setObject:filePath forKey:@"headPhoto_tea"];
    [userDe synchronize];
    // 保存文件的名称
