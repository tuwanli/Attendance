...
#import "SPersonVC.h"
#import "LoginVC.h"
@interface SPersonVC ()<UIGestureRecognizerDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
- (void)viewDidLoad {
    ...
    NSString *headImagePath = [[NSUserDefaults standardUserDefaults]objectForKey:@"headPhoto"];
    [_headImageView setImage:[UIImage imageNamed:@"testIcon"]];
    if (headImagePath&&headImagePath.length>0) {
        [_headImageView setImage:[self takepicture:[NSString stringWithFormat:@"%ld",userid]]];
    }
   
    
    
    [self.nameL setText:model.student_name];
    [self.yuanxuL setText:model.student_department];
    [self.classL setText:model.student_class];
    [self.phoneL setText:model.student_phone];
    [self.studySoreL setText:@"1.5"];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [self saveImage:tempImage name:[NSString stringWithFormat:@"%ld",userid]];
}
//修改图像的尺寸
...
- (void)saveImage:(UIImage *)image name:(NSString *)iconName
{
    ...
    [userDe setObject:filePath forKey:@"headPhoto"];
    [userDe synchronize];
    // 保存文件的名称
